...
source("function.R")
##Load Data
best_lambda=as.matrix(read.csv("best_lambda1_1.csv")$lambda)
...
MMSE_1[4:6]<-MMSE[,2]
MMSE_1[7:9]<-MMSE[,3]
Beta=data.frame("beta"=rep(c("1","2","3"),9),"y"=c(MAB_1[1:9],MSD_1[1:9],MMSE_1[1:9]),group1=c(rep("MAB",9),rep("MSD",9),rep("MMSE",9)),group2=rep("design1",27),"lambda"=rep(c(rep("0",3),rep("1",3),rep("from mDIC",3)),3))
...
Eta=data.frame("eta"=rep(c("1","2","3"),9),"y"=c(MAB_Eta_1,MSD_Eta_1,MMSE_Eta_1),group1=c(rep("MAB",9),rep("MSD",9),rep("MMSE",9)),group2=rep("design1",27),"lambda"=rep(c(rep("0",3),rep("1",3),rep("from mDIC",3)),3))
...
means <- data.frame("lambda"=c(0,1,"LPML"),randindex=c(m1,m2,sprintf("%.4f",m3)))
