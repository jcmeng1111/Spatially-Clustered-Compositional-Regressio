...
X1[,1:16]=data[,3:18]
X3=matrix(0,51,3)
##Merge the industry components into three major components
X3[,1]=X1[,1]
X3[,2]=rowSums(X1[,2:5])
...
##set sampleN=1,step=0:10 to select lambda
##set sampleN=1,step=7 to get the estimation of beta
##set sampleN=1：100,step=7 to run 100 MCMC chains
for(sampleN in 1:1){
    for(step in 0:10){
    RD<-rep(0,100)
    set.seed(sampleN+99)
    ...
    for(i.iter in 1:n.MCMC.iter)
    {
      ...
      if(i.iter>n.MCMC.iter-nburn.in){
        j=i.iter-(n.MCMC.iter-nburn.in)
...
stateinfo$cluster <- as.factor(1*(stateinfo$abbr %in% c0[1,])+2*(stateinfo$abbr %in% c0[2,])+3*(stateinfo$abbr %in% c0[3,])+4*(stateinfo$abbr %in% c0[4,])+5*(stateinfo$abbr %in% c0[5,])+6*(stateinfo$abbr %in% c0[6,])+7*(stateinfo$abbr %in% c0[7,])+8*(stateinfo$abbr %in% c0[8,])+9*(stateinfo$abbr %in% c0[9,])+10*(stateinfo$abbr %in% c0[10,])+11*(stateinfo$abbr %in% c0[11,]))
